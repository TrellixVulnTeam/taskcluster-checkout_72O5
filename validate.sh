...
[ -z "$VIRTUAL_ENV" ] && fail "Need an activated virtualenv with taskcluster-checkout installed"

tmpbase=$(mktemp -d -t tmpbase.XXXXXX)
trap 'rm -rf ${tmpbase}; exit 1' 1 2 3 15

status "running pep8"
pep8 --config=pep8rc checkout.py || not_ok "pep8 failed"

status "running pyflakes"
pyflakes checkout.py || not_ok "pyflakes failed"

status "running tests (under coverage)"
coverage erase || not_ok "coverage failed"
coverage run --rcfile=coveragerc --source=checkout $VIRTUAL_ENV/bin/nosetests test_checkout.py || not_ok "tests failed"
